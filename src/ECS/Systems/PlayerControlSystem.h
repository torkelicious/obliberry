    ...
    inline void Update(Registry &registry, const Core::EngineContext &ctx) noexcept {
        ...
        if (ctx.camera && ctx.window && ctx.input) {
            worldPos = ctx.camera->MouseToWorld(
                    static_cast<float>(ctx.input->MousePosX()), static_cast<float>(ctx.input->MousePosY()), static_cast<float>(ctx.window->GetWidth()), static_cast<float>(ctx.window->GetHeight()));
        }
