...
void Scripting::EngineLib::register_camera_modules(ObSL::Interpreter &interpreter) {
    ...
    interpreter.get_global_environment()->define(
            ...
            interpreter.gc.allocate<ObSL::NativeFunction>(
                    ...
                    [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                        if (ctx && ctx->camera && args.size() >= 3 && std::holds_alternative<double>(args[0]) &&
    ...
    interpreter.get_global_environment()->define(
            ...
            interpreter.gc.allocate<ObSL::NativeFunction>(
                    2,
                    [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                        if (ctx && ctx->camera && args.size() >= 2 && std::holds_alternative<double>(args[0]) &&
                            std::holds_alternative<double>(args[1])) {
                            std::lock_guard lock(s_CameraMutex);
                            glm::vec2 screenPan(static_cast<float>(std::get<double>(args[0])),
                            ...
                            ctx->camera->Position +=
                                    glm::vec3(worldPan.x, worldPan.y, 0.0f) * (1.0f / ctx->camera->Zoom);
                            return true;
                        }
                        return false;
                    },
            ...
            interpreter.gc.allocate<ObSL::NativeFunction>(
                    ...
                    [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                        if (ctx && ctx->camera && args.size() >= 2 && std::holds_alternative<double>(args[0]) &&
                            std::holds_alternative<double>(args[1])) {
                            std::lock_guard lock(s_CameraMutex);
                            ctx->camera->SetRotation(static_cast<float>(std::get<double>(args[0])),
                                                     static_cast<float>(std::get<double>(args[1])));
                            return true;
                        }
                        return false;
