...
void Scripting::EngineLib::register_time_modules(ObSL::Interpreter &interpreter) {
    interpreter.get_global_environment()->define("GetFrameCount", interpreter.gc.allocate<ObSL::NativeFunction>(0, [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &) -> ObSL::Value {
        if (ctx) {
    ...
    interpreter.get_global_environment()->define("GetTimeScale", interpreter.gc.allocate<ObSL::NativeFunction>(0, [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &) -> ObSL::Value {
        if (ctx) {
            std::lock_guard lock(s_TimeMutex);
    ...
    interpreter.get_global_environment()->define("SetTimeScale", interpreter.gc.allocate<ObSL::NativeFunction>(1, [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
    ...
    interpreter.get_global_environment()->define("GetRawDt",
