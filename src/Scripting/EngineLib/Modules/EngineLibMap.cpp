...
void Scripting::EngineLib::register_map_modules(ObSL::Interpreter &interpreter) {
                        ...
                        if (cmd_buf) {
                            cmd_buf->push([q, r](ECS::Registry &reg) {
                                ...
                                reg.ForEach<ECS::Components::MapComponent>(
                                        [&](ECS::Entity, ECS::Components::MapComponent *map) {
                    ...
                    [reg = m_registry](const ObSL::Interpreter *interpreter,
                        ...
                        if (cmd_buf) {
                            cmd_buf->push([id, q, r](ECS::Registry &reg) {
                                ...
                                reg.ForEach<ECS::Components::MapComponent>(
                                        [&](ECS::Entity, const ECS::Components::MapComponent *map) { mapComp = map; });
    ...
    interpreter.get_global_environment()->define(
            ...
            interpreter.gc.allocate<ObSL::NativeFunction>(
                    ...
                    [reg = m_registry](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                        ...
                        std::shared_lock lock(g_RegistryMutex);
                        const Map::HexCoords pos(static_cast<int32_t>(std::get<double>(args[0])),
    ...
    // Hex helpers
    interpreter.get_global_environment()->define(
            "Hex_Distance", interpreter.gc.allocate<ObSL::NativeFunction>(
                                    4,
                                    [](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                                        if (args.size() < 4 || !std::holds_alternative<double>(args[0]) ||
                                            !std::holds_alternative<double>(args[1]) ||
                                            !std::holds_alternative<double>(args[2]) ||
                                            !std::holds_alternative<double>(args[3]))
                                            return 0.0;
                                        const Map::HexCoords a(static_cast<int32_t>(std::get<double>(args[0])),
                                                               static_cast<int32_t>(std::get<double>(args[1])));
                                        const Map::HexCoords b(static_cast<int32_t>(std::get<double>(args[2])),
                                                               static_cast<int32_t>(std::get<double>(args[3])));
            ...
            interpreter.gc.allocate<ObSL::NativeFunction>(
                    ...
                    [](ObSL::Interpreter *interp, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                        if (args.size() < 2 || !std::holds_alternative<double>(args[0]) ||
                            !std::holds_alternative<double>(args[1]))
                            return std::monostate{};
                        const Map::HexCoords hex(static_cast<int32_t>(std::get<double>(args[0])),
                                                 static_cast<int32_t>(std::get<double>(args[1])));
