...
void Scripting::EngineLib::register_map_modules(ObSL::Interpreter &interpreter) {
    // HEX MATH
    interpreter.get_global_environment()->define("Math_WorldToHex", interpreter.gc.allocate<ObSL::NativeFunction>(2, [](ObSL::Interpreter *interp, const std::vector<ObSL::Value> &args) -> ObSL::Value {
    ...
    interpreter.get_global_environment()->define("GetSelectedHex", interpreter.gc.allocate<ObSL::NativeFunction>(0, [reg = m_registry](ObSL::Interpreter *interp, const std::vector<ObSL::Value> &) -> ObSL::Value {
    ...
    interpreter.get_global_environment()->define(
            "SetSelectedHex", interpreter.gc.allocate<ObSL::NativeFunction>(2, [reg = m_registry](const ObSL::Interpreter *interpreter, const std::vector<ObSL::Value> &args) -> ObSL::Value {
        if (args.size() < 2 || !std::holds_alternative<double>(args[0]) || !std::holds_alternative<double>(args[1])) {
    ...
    interpreter.get_global_environment()->define(
            "SetPathToHex", interpreter.gc.allocate<ObSL::NativeFunction>(3, [reg = m_registry](const ObSL::Interpreter *interpreter, const std::vector<ObSL::Value> &args) -> ObSL::Value {
    ...
    interpreter.get_global_environment()->define(
            "ClearSelectionOverlay", interpreter.gc.allocate<ObSL::NativeFunction>(0, [reg = m_registry](const ObSL::Interpreter *interpreter, const std::vector<ObSL::Value> &) -> ObSL::Value {
        auto *worker = static_cast<ObSL::ScriptWorker *>(interpreter->user_data);
        auto *cmd_buf = worker->frame_context<ScriptCommandBuffer>();
        if (cmd_buf) {
    ...
    interpreter.get_global_environment()->define(
            "ClearPathTarget", interpreter.gc.allocate<ObSL::NativeFunction>(0, [reg = m_registry](const ObSL::Interpreter *interpreter, const std::vector<ObSL::Value> &) -> ObSL::Value {
        auto *worker = static_cast<ObSL::ScriptWorker *>(interpreter->user_data);
        auto *cmd_buf = worker->frame_context<ScriptCommandBuffer>();
        if (cmd_buf) {
            cmd_buf->push([](ECS::Registry &reg) {
    ...
    interpreter.get_global_environment()->define("Map_IsHexWalkable", interpreter.gc.allocate<ObSL::NativeFunction>(2, [reg = m_registry](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
        if (args.size() < 2 || !std::holds_alternative<double>(args[0]) || !std::holds_alternative<double>(args[1])) {
            return false;
        }
        std::shared_lock lock(g_RegistryMutex);
    ...
    interpreter.get_global_environment()->define("Map_GetMapEntity", interpreter.gc.allocate<ObSL::NativeFunction>(0, [reg = m_registry](ObSL::Interpreter *interp, const std::vector<ObSL::Value> &) -> ObSL::Value {
        std::shared_lock lock(g_RegistryMutex);
        if (const ECS::EntityID mapId = reg->FindFirstEntity<ECS::Components::MapComponent>(); mapId != ECS::INVALID_ENTITY_ID) {
            return CreateEntityObjectLocked(interp, *reg, mapId);
        }
        return std::monostate{};
    }, "Map_GetMapEntity"));

    // Hex helpers
    interpreter.get_global_environment()->define("Hex_Distance", interpreter.gc.allocate<ObSL::NativeFunction>(4, [](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
        if (args.size() < 4 || !std::holds_alternative<double>(args[0]) || !std::holds_alternative<double>(args[1]) || !std::holds_alternative<double>(args[2]) || !std::holds_alternative<double>(args[3]))
            return 0.0;
    ...
    interpreter.get_global_environment()->define("Hex_GetNeighbors", interpreter.gc.allocate<ObSL::NativeFunction>(2, [](ObSL::Interpreter *interp, const std::vector<ObSL::Value> &args) -> ObSL::Value {
        if (args.size() < 2 || !std::holds_alternative<double>(args[0]) || !std::holds_alternative<double>(args[1]))
            return std::monostate{};
    ...
    interpreter.get_global_environment()->define("Hex_HexToWorld", interpreter.gc.allocate<ObSL::NativeFunction>(2, [](ObSL::Interpreter *interp, const std::vector<ObSL::Value> &args) -> ObSL::Value {
        auto *obj = interp->gc.allocate<ObSL::ObSLObject>();
        if (args.size() < 2 || !std::holds_alternative<double>(args[0]) || !std::holds_alternative<double>(args[1])) {
            obj->fields["x"] = 0.0;
            obj->fields["y"] = 0.0;
    ...
    // muts
    interpreter.get_global_environment()->define("Map_SetHexWalkable", interpreter.gc.allocate<ObSL::NativeFunction>(2, // obj , bool
    ...
    interpreter.get_global_environment()->define("Map_SetTileType", interpreter.gc.allocate<ObSL::NativeFunction>(2, // obj ,num
                                                                            [reg = m_registry](const ObSL::Interpreter *interpreter, const std::vector<ObSL::Value> &args) -> ObSL::Value {
