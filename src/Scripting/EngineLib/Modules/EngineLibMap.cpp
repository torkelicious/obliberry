...
void Scripting::EngineLib::register_map_modules(ObSL::Interpreter &interpreter) {
            ...
            [reg = m_registry](const ObSL::Interpreter *interpreter,
                ...
                if (cmd_buf) {
                    cmd_buf->push([id, q, r](ECS::Registry &reg) {
                        ...
                        if (!move || !trans) return;

                        const ECS::Components::MapComponent *mapComp = nullptr;
                        reg.ForEach<ECS::Components::MapComponent>(
                            [&](ECS::Entity, const ECS::Components::MapComponent *map) {
                                mapComp = map;
                            });
        ...
        "Hex_Distance", interpreter.gc.allocate<ObSL::NativeFunction>(
            ...
            [](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                if (args.size() < 4
                    ...
                    return 0.0;
                const Map::HexCoords a(
                    static_cast<int32_t>(std::get<double>(args[0])),
                    static_cast<int32_t>(std::get<double>(args[1])));
                const Map::HexCoords b(
                    static_cast<int32_t>(std::get<double>(args[2])),
                    static_cast<int32_t>(std::get<double>(args[3])));
                return static_cast<double>(Math::HexMath::Distance(a, b));
            }, "Hex_Distance"));

    interpreter.get_global_environment()->define(
        "Hex_GetNeighbors", interpreter.gc.allocate<ObSL::NativeFunction>(
            2,
            [](ObSL::Interpreter *interp, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                if (args.size() < 2
                    || !std::holds_alternative<double>(args[0])
    ...
    interpreter.get_global_environment()->define(
        "Hex_HexToWorld", interpreter.gc.allocate<ObSL::NativeFunction>(
            2,
            [](ObSL::Interpreter *interp, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                auto *obj = interp->gc.allocate<ObSL::ObSLObject>();
                if (args.size() < 2
