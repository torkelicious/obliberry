...
void Scripting::EngineLib::register_input_modules(ObSL::Interpreter &interpreter) {
    // KEYBOARD
    interpreter.get_global_environment()->define("Input_IsKeyDown", interpreter.gc.allocate<ObSL::NativeFunction>(
                                                                            1,
    ...
    interpreter.get_global_environment()->define("Input_IsKeyPressed", interpreter.gc.allocate<ObSL::NativeFunction>(
                                                                               1,
                                                                               [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                                                                                   if (!ctx || !ctx->input || args.empty() || !std::holds_alternative<std::string>(args[0]))
                                                                                       return false;
    ...
    interpreter.get_global_environment()->define("Input_IsKeyReleased", interpreter.gc.allocate<ObSL::NativeFunction>(
                                                                                1,
                                                                                [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                                                                                    if (!ctx || !ctx->input || args.empty() || !std::holds_alternative<std::string>(args[0]))
                                                                                        return false;
    ...
    // MOUSE BUTTON
    interpreter.get_global_environment()->define("Input_IsMouseDown", interpreter.gc.allocate<ObSL::NativeFunction>(
                                                                              1,
                                                                              [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
    ...
    interpreter.get_global_environment()->define("Input_IsMousePressed", interpreter.gc.allocate<ObSL::NativeFunction>(
                                                                                 1,
                                                                                 [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                                                                                     if (!ctx || !ctx->input || args.empty() || !std::holds_alternative<double>(args[0]))
                                                                                         return false;
    ...
    interpreter.get_global_environment()->define("Input_IsMouseReleased", interpreter.gc.allocate<ObSL::NativeFunction>(
                                                                                  1,
                                                                                  [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &args) -> ObSL::Value {
                                                                                      if (!ctx || !ctx->input || args.empty() || !std::holds_alternative<double>(args[0]))
                                                                                          return false;
    ...
    interpreter.get_global_environment()->define(
            "Input_GetMouseX", interpreter.gc.allocate<ObSL::NativeFunction>(
                                       0, [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &) -> ObSL::Value { return ctx && ctx->input ? ctx->input->MousePosX() : 0.0; }, "Input_GetMouseX"));

    interpreter.get_global_environment()->define(
            "Input_GetMouseY", interpreter.gc.allocate<ObSL::NativeFunction>(
                                       0, [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &) -> ObSL::Value { return ctx && ctx->input ? ctx->input->MousePosY() : 0.0; }, "Input_GetMouseY"));

    interpreter.get_global_environment()->define(
            "Input_GetScrollX", interpreter.gc.allocate<ObSL::NativeFunction>(
                                        0, [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &) -> ObSL::Value { return ctx && ctx->input ? ctx->input->ScrollX() : 0.0; }, "Input_GetScrollX"));

    interpreter.get_global_environment()->define(
            "Input_GetScrollY", interpreter.gc.allocate<ObSL::NativeFunction>(
                                        0, [ctx = m_ctx](ObSL::Interpreter *, const std::vector<ObSL::Value> &) -> ObSL::Value { return ctx && ctx->input ? ctx->input->ScrollY() : 0.0; }, "Input_GetScrollY"));

    interpreter.get_global_environment()->define("Input_GetMouseWorldPos", interpreter.gc.allocate<ObSL::NativeFunction>(
                                                                                   0,
