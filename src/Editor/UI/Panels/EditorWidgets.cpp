...
namespace {
    ...
    std::optional<std::string> ResolveOrImportPath(
...
void Editor::UI::MeshWidget::Draw(const ECS::Entity entity, Core::EngineContext *engineContext) {
    ...
    if (ImGui::CollapsingHeader(GetName())) {
        ...
        constexpr const char *meshTypes[] = {
            "Quad", "PointTopHex", "ETriang", "Ellipse", "Circle",
            "Pentagon", "Hexagon", "Octagon", "Ring", "Sector", "Diamond"
        };
        ...
        if (ImGui::Button("Assign")) {
            ...
            switch (m_SelectedMesh) {
                case 0: data = Rendering::MeshFactory::CreateQuad();
                    break;
                case 1: data = Rendering::MeshFactory::CreatePointTopHex();
                    break;
                case 2: data = Rendering::MeshFactory::CreateEquiTriangle(0.5f);
                    break;
                case 3: data = Rendering::MeshFactory::CreateEllipse();
                    break;
                case 4: data = Rendering::MeshFactory::CreateEllipse();
                    break;
                case 5: data = Rendering::MeshFactory::CreateRegularPolygon(5);
                    break;
                case 6: data = Rendering::MeshFactory::CreateRegularPolygon(6);
                    break;
                case 7: data = Rendering::MeshFactory::CreateRegularPolygon(8);
                    break;
                case 8: data = Rendering::MeshFactory::CreateRing();
                    break;
                case 9: data = Rendering::MeshFactory::CreateSector();
                    break;
                case 10: data = Rendering::MeshFactory::CreateDiamond();
                    break;
            }
...
void Editor::UI::MaterialWidget::Draw(const ECS::Entity entity, Core::EngineContext *engineContext) {
    ...
    if (ImGui::CollapsingHeader(GetName())) {
        ...
        if (comp->material) {
            ...
            ImGui::Text("Frag: %s", comp->material->shader
                                        ? comp->material->shader->GetFragmentPath().c_str()
                                        : "None");
            ImGui::SameLine();
            ...
            if (ImGui::Button("Compile Shaders")) {
                if (comp->material->shader) {
                    auto shader = std::make_shared<Rendering::Shader>(
                        comp->material->shader->GetVertexPath(),
        ...
        ImGui::SetCursorPosX(
            ImGui::GetCursorPosX() + (ImGui::GetContentRegionAvail().x - (
                                          ImGui::CalcTextSize("Remove Material").x + ImGui::GetStyle().FramePadding.x *
                                          2)) * 0.5f);
...
void Editor::UI::DirectionalTextureWidget::Draw(const ECS::Entity entity, Core::EngineContext *engineContext) {
    ...
    if (ImGui::CollapsingHeader(GetName())) {
        ...
        ImGui::Separator();
        const float buttonWidth = ImGui::CalcTextSize("Remove Directional Texture").x + ImGui::GetStyle().FramePadding.x
                                  * 2;
        const float availWidth = ImGui::GetContentRegionAvail().x;
        ImGui::SetCursorPosX(ImGui::GetCursorPosX() + (availWidth - buttonWidth) * 0.5f);
...
void Editor::UI::CustomDataWidget::Draw(const ECS::Entity entity, Core::EngineContext *engineContext) {
    ...
    if (ImGui::CollapsingHeader(GetName())) {
        ...
        ImGui::Separator();
        const float buttonWidth = ImGui::CalcTextSize("Remove ObSL Custom Data").x + ImGui::GetStyle().FramePadding.x *
                                  2;
        const float availWidth = ImGui::GetContentRegionAvail().x;
        ImGui::SetCursorPosX(ImGui::GetCursorPosX() + (availWidth - buttonWidth) * 0.5f);
