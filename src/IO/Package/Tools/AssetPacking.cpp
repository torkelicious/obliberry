    ...
    bool pack_one_file(const std::filesystem::path &filepath, const std::filesystem::path &project_dir, const std::filesystem::path &script_root, ContainerWriter &writer, DependencyGraph &dep_graph,
        ...
        if (ext == ".obsl") {
            ...
            if (opts.verbose && !opts.quiet)
                LOG_INFO(LOG_WHO, "[SCRIPT]  " + canonical_path);
        } else if (ext == ".json") {
            std::string source_code = read_file_string(filepath);
            ...
            if (opts.verbose && !opts.quiet)
                LOG_INFO(LOG_WHO, "[JSON]    " + canonical_path);
            ...
            if (opts.verbose && !opts.quiet)
                LOG_INFO(LOG_WHO, "[SHADER]  " + canonical_path);
        } else if (ext == ".obmap") {
            auto raw_data = read_file_binary(filepath);
            writer.add_raw_data(canonical_path, std::move(raw_data), EntryType::RawBinary, opts.global_compress);
            if (opts.verbose && !opts.quiet)
                LOG_INFO(LOG_WHO, "[OBMAP]   " + canonical_path);
        } else {
            auto raw_data = read_file_binary(filepath);
            writer.add_raw_data(canonical_path, std::move(raw_data), EntryType::RawBinary, opts.global_compress);
            if (opts.verbose && !opts.quiet)
                LOG_INFO(LOG_WHO, "[RAW]     " + canonical_path);
        }
