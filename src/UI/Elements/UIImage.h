...
public:
  void SetImage(const std::shared_ptr<Rendering::Texture> &tex) {
    m_Image = tex;
  }
  void SetColor(const glm::vec4 color) { m_Color = color; }
  [[nodiscard]] const glm::vec4 &GetColor() const { return m_Color; }
  [[nodiscard]] const std::shared_ptr<Rendering::Texture> &GetImage() const {
    return m_Image;
  }
  [[nodiscard]] std::shared_ptr<Rendering::Texture> &GetImage() {
    return m_Image;
  }
