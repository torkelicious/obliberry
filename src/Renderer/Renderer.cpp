...
void Renderer::Flush(const size_t renderIndex) {
    if (!m_Commands[renderIndex].empty()) {
        std::ranges::sort(m_Commands[renderIndex], [](const RenderCommand &a, const RenderCommand &b) {
            ...
            if (const auto zB = static_cast<int16_t>(b.sortKey & 0xFFFF); zA != zB) return zA < zB;
            if (depthA != depthB) return depthA > depthB;
            if (a.material != b.material) return a.material < b.material;
            if (a.effectiveTexture != b.effectiveTexture) return a.effectiveTexture < b.effectiveTexture;
    ...
    // merge instanced commands by sorting first
    {
        ...
        std::ranges::sort(entries, [](const InstancedEntry &a, const InstancedEntry &b) {
            if (a.key.mesh != b.key.mesh) return a.key.mesh < b.key.mesh;
            if (a.key.material != b.key.material) return a.key.material < b.key.material;
            if (a.key.texture != b.key.texture) return a.key.texture < b.key.texture;
            if (a.key.color.r != b.key.color.r) return a.key.color.r < b.key.color.r;
            if (a.key.color.g != b.key.color.g) return a.key.color.g < b.key.color.g;
            if (a.key.color.b != b.key.color.b) return a.key.color.b < b.key.color.b;
...
void Renderer::RenderBatch(const BatchKey &key, const std::vector<glm::mat4> &transforms, const size_t renderIndex) {
    ...
    if (instanceCount == 0 || instanceCount > MAX_INSTANCES) return;
    ...
    m_DynamicInstanceBuffer->SetDataOrphaned(transforms.data(),
...
void Renderer::SetLightmap(const Lightmap *lightmap) {
    m_Lightmap[m_SubmitIndex] = lightmap;
}
