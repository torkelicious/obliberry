...
A **scene** is a self-contained level or screen in your project. It holds everything that exists together at one time: the map, entities, UI, lighting, music, and asset references.
...
> [!NOTE]
> Scenes are saved as JSON files under `assets/scenes/` in your project. The format is documented in [scene-json.md](../formats/scene-json.md) if you need the technical details.
...
You don't edit these components directly. Use **Map Edit mode** to paint the map, and the MAP entity updates automatically.

> [!IMPORTANT]
> The map file path is stored *in the scene*, not in the MAP entity. When you save the scene, the map reference is saved with it.
