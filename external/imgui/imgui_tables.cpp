...
bool ImGui::BeginTableEx(const char *name, ImGuiID id, int columns_count, ImGuiTableFlags flags,
    ...
    const ImVec2 actual_outer_size = ImTrunc(CalcItemSize(outer_size, ImMax(avail_size.x, IMGUI_WINDOW_HARD_MIN_SIZE),
                                                          use_child_window
                                                              ? ImMax(avail_size.y, IMGUI_WINDOW_HARD_MIN_SIZE)
                                                              : 0.0f));
    const ImRect outer_rect(outer_window->DC.CursorPos, outer_window->DC.CursorPos + actual_outer_size);
    const bool outer_window_is_measuring_size = (outer_window->AutoFitFramesX > 0) || (
                                                    outer_window->AutoFitFramesY > 0);
    // Doesn't apply to AlwaysAutoResize windows!
    ...
    if (use_child_window) {
        ...
        // This is designed to always linking ImGuiTreeNodeFlags_DrawLines linking across a table
    }
...
static int TableGetMaxDisplayOrderAllowed(ImGuiTable *table, int src_order, int dst_order) {
    ...
    for (int order_n = src_order; (src_order < dst_order && order_n <= dst_order) || (
                                      dst_order < src_order && order_n >= dst_order); order_n += reorder_dir)
...
void ImGui::TableUpdateLayout(ImGuiTable *table) {
    ...
    const float width_removed = (table->HasScrollbarYPrev && !table->InnerWindow->ScrollbarY)
                                    ...
                                    : 0.0f;
    // To synchronize decoration width of synced tables with mismatching scrollbar state (#5920)
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthStretch) {
            ...
            column->WidthRequest = IM_TRUNC(ImMax(width_avail_for_stretched_columns * weight_ratio,
    ...
    if (table->FreezeColumnsRequest > 0)
        table->InnerWindow->DecoInnerSizeX1 = table->Columns[table->DisplayOrderToIndex[
                                                  table->FreezeColumnsRequest - 1]].MaxX - table->OuterRect.Min.x;
    // FIXME-FROZEN
...
void ImGui::EndTable() {
    ...
    if (table->Flags & ImGuiTableFlags_NoHostExtendX) {
        ...
        const float decoration_size = table->TempData->AngledHeadersExtraWidth + ((inner_window != outer_window)
...
void ImGui::TableSetupColumn(const char *label, ImGuiTableColumnFlags flags, float init_width_or_weight,
    ...
    // Table layout is locked when submitting a row or when calling BeginMultiSelect() with box-select.
    IM_ASSERT(
...
void ImGui::TableSetColumnWidth(int column_n, float width) {
    ...
    // Don't use TableCalcMaxColumnWidth() here as it would rely on MinX from last instance (#7933)
...
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle, float max_label_width,
    ...
    // FIXME: Standardize those scaling factors better
...
static bool MenuItemForColumnReorder(const char *label, bool selected, bool enabled) {
    ...
    // Can't use IsMouseDragging(0) as button is released already.
...
void ImGui::TableDrawDefaultContextMenu(ImGuiTable *table, ImGuiTableFlags flags_for_section_to_display) {
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Resizable) {
        if (context_column != NULL) {
            const bool can_resize = !(context_column->Flags & ImGuiTableColumnFlags_NoResize) && context_column->
                                    IsEnabled;
...
#if 0
    if ((flags_for_section_to_display & ImGuiTableFlags_Sortable) && column != NULL && (
    ...
    if (flags_for_section_to_display & ImGuiTableFlags_Hideable) {
        ...
        const bool is_reordering = (g.ActiveId != 0 && g.ActiveIdWindow == g.CurrentWindow && table->ReorderColumn != -1
        ...
        const int reorder_max_order = is_reordering
        ...
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
            ...
            if (IsItemActive() && IsMouseDragging(0) && g.ActiveIdSource == ImGuiInputSource_Mouse && (
                ...
                // Disable toggle in MenuItemForColumnReorder() + start dimming to display allowed reorder targets.
                table->ReorderColumn = (ImGuiTableColumnIdx) column_n;
                if (!IsItemHovered()) {
                    int reorder_dir = (g.IO.MousePos.y < (g.LastItemData.Rect.Min.y + g.LastItemData.Rect.Max.y) * 0.5f)
                                          ? -1
                                          : +1;
                    float reorder_amount = (reorder_dir < 0
                                                ? g.LastItemData.Rect.Min.y - g.IO.MousePos.y
                                                : g.IO.MousePos.y - g.LastItemData.Rect.Max.y) / g.LastItemData.Rect.
                                           GetHeight();
...
void ImGui::TableFixDisplayOrder(ImGuiTable *table) {
    ...
    // FIXME: Maybe wrap those two lines as a helper.
    ImGuiTableFixDisplayOrderColumnData *fdo_columns = (ImGuiTableFixDisplayOrderColumnData *) (void *) g.TempBuffer.
            Data;
    for (int n = 0; n < table->ColumnsCount; n++) {
...
void ImGui::DebugNodeTable(ImGuiTable *table) {
    ...
    if (IsItemHovered())
        GetForegroundDrawList(table->OuterWindow)->AddRect(table->OuterRect.Min, table->OuterRect.Max,
                                                           IM_COL32(255, 255, 0, 255));
    if (IsItemVisible() && table->HoveredColumnBody != -1)
        GetForegroundDrawList(table->OuterWindow)->AddRect(GetItemRectMin(), GetItemRectMax(),
                                                           IM_COL32(255, 255, 0, 255));
    if (!open)
        return;
    if (table->InstanceCurrent > 0)
