...
static void ImGui_ImplOpenGL3_DrawCallback_SetSamplerLinear(const ImDrawList *, const ImDrawCmd *) {
    ImGui_ImplOpenGL3_Data *bd = ImGui_ImplOpenGL3_GetBackendData();
    if (bd->HasBindSampler) { glBindSampler(0, bd->TexSamplers[0]); } else {
...
static void ImGui_ImplOpenGL3_DrawCallback_SetSamplerNearest(const ImDrawList *, const ImDrawCmd *) {
    ImGui_ImplOpenGL3_Data *bd = ImGui_ImplOpenGL3_GetBackendData();
    if (bd->HasBindSampler) { glBindSampler(0, bd->TexSamplers[1]); } else {
        bd->UseTexParameterToSetSampler = true;
...
void ImGui_ImplOpenGL3_RenderDrawData(ImDrawData *draw_data) {
...
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_BIND_SAMPLER
    GLuint last_sampler; if (bd->HasBindSampler) { glGetIntegerv(GL_SAMPLER_BINDING, (GLint *) &last_sampler); } else {
        last_sampler = 0;
    }
#endif
    ...
    for (const ImDrawList *draw_list: draw_data->CmdLists) {
        ...
        if (bd->UseBufferSubData) {
            ...
            GL_CALL(
                glBufferData(GL_ELEMENT_ARRAY_BUFFER, idx_buffer_size, (const GLvoid*)draw_list->IdxBuffer.Data,
                    GL_STREAM_DRAW));
        }
...
static void ImGui_ImplOpenGL3_DestroyTexture(ImTextureData *tex) {
    GLuint gl_tex_id = (GLuint)(intptr_t)
    tex->TexID;
...
bool ImGui_ImplOpenGL3_CreateDeviceObjects() {
    ...
    if (bd->HasBindSampler) {
        ...
        for (int sampler_n = 0; sampler_n < 2; sampler_n++) {
            GL_CALL(
                glSamplerParameteri(bd->TexSamplers[sampler_n], GL_TEXTURE_MIN_FILTER, (sampler_n == 0) ? GL_LINEAR :
                    GL_NEAREST));
            GL_CALL(
                glSamplerParameteri(bd->TexSamplers[sampler_n], GL_TEXTURE_MAG_FILTER, (sampler_n == 0) ? GL_LINEAR :
                    GL_NEAREST));
