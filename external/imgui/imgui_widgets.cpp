...
void ImGui::TextAlignedV(float align_x, float size_x, const char *fmt, va_list args) {
    ...
    // Cancel out extending content size because right-aligned text would otherwise mess it up.

    if (size_x < text_size.x && IsItemHovered(
            ImGuiHoveredFlags_NoNavOverride | ImGuiHoveredFlags_AllowWhenDisabled | ImGuiHoveredFlags_ForTooltip))
...
bool ImGui::ButtonBehavior(const ImRect &bb, ImGuiID id, bool *out_hovered, bool *out_held, ImGuiButtonFlags flags) {
    ...
    const bool flatten_hovered_children = (flags & ImGuiButtonFlags_FlattenChildren) && g.HoveredWindow && g.
                                          HoveredWindow->RootWindow == window->RootWindow;
    ...
    if (g.DragDropActive) {
        if ((flags & ImGuiButtonFlags_PressedOnDragDropHold) && !(
                g.DragDropSourceFlags & ImGuiDragDropFlags_SourceNoHoldToOpenOthers) && IsItemHovered(
            ...
            if (flags & ImGuiButtonFlags_PressedOnRelease) {
                if (mouse_button_released != -1) {
                    const bool has_repeated_at_least_once =
                            (item_flags & ImGuiItemFlags_ButtonRepeat) && g.IO.MouseDownDurationPrev[
...
void ImGui::ExtendHitBoxWhenNearViewportEdge(ImGuiWindow *window, ImRect *bb, float threshold, ImGuiAxis axis) {
    ...
    if (window_rect.Min[axis] == viewport_rect.Min[axis] && bb->Min[axis] > window_rect.Min[axis] && bb->Min[axis] -
        threshold <= window_rect.Min[axis])
        bb->Min[axis] = window_rect.Min[axis];
    if (window_rect.Max[axis] == viewport_rect.Max[axis] && bb->Max[axis] < window_rect.Max[axis] && bb->Max[axis] +
        threshold >= window_rect.Max[axis])
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE *v, float v_speed, const TYPE v_min, const TYPE v_max,
    ...
    const bool is_bounded = (v_min < v_max) || ((v_min == v_max) && (
...
bool ImGui::DragScalar(const char *label, ImGuiDataType data_type, void *p_data, float v_speed, const void *p_min,
    ...
    char value_buf[64];
    const char *value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_COUNTOF(value_buf), data_type, p_data,
                                                                 format);
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
...
bool ImGui::SliderScalar(const char *label, ImGuiDataType data_type, void *p_data, const void *p_min, const void *p_max,
    ...
    if (!temp_input_is_active) {
        ...
        if (make_active && clicked)
            SetKeyOwner(ImGuiKey_MouseLeft, id);
        if (make_active && temp_input_allowed)
            if ((clicked && g.IO.KeyCtrl) || (g.NavActivateId == id && (
                                                  g.NavActivateFlags & ImGuiActivateFlags_PreferInput)))
                temp_input_is_active = true;
...
bool ImGui::TempInputText(const ImRect &bb, ImGuiID id, const char *label, char *buf, size_t buf_size,
    ...
    // Using ImGuiInputTextFlags_MergedItem above will skip ItemAdd() so we poke here.
    bool value_changed = InputTextEx(label, NULL, buf, (int) buf_size, bb.GetSize(),
...
bool ImGui::TempInputScalar(const ImRect &bb, ImGuiID id, const char *label, ImGuiDataType data_type, void *p_data,
    ...
    // Because TempInputText() uses ImGuiInputTextFlags_MergedItem it doesn't submit a new item, so we poke LastItemData.
...
bool ImGui::InputScalar(const char *label, ImGuiDataType data_type, void *p_data, const void *p_step,
    ...
    // We would be using 'ret' if ImGuiInputTextFlags_EnterReturnsTrue was not involved.
    bool value_changed = input_edited
...
namespace ImStb {
    ...
    static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow *r, ImGuiInputTextState *obj, int line_start_idx) {
        ...
        const ImVec2 size = InputTextCalcTextSize(obj->Ctx, text + line_start_idx, text + obj->TextLen,
    ...
    static int STB_TEXTEDIT_MOVELINESTART_IMPL(ImGuiInputTextState *obj, ImStb::STB_TexteditState *state, int cursor) {
        ...
        if (obj->WrapWidth > 0.0f) {
            ...
            while (p >= p_bol) {
                ...
                if (p == p_cursor)
                    // If we are already on a visible beginning-of-line, return real beginning-of-line (would be same as regular handler below)
        ...
        if (obj->WrapWidth > 0.0f) {
            ...
            while (p < text_end) {
                ...
                if (p_eol == p_cursor && obj->LastMoveDirectionLR != ImGuiDir_Left)
                    // If we are already on a visible end-of-line, switch to regular handle
                    break;
                if (p_eol > p_cursor)
                    return cursor;
                p = (*p_eol == '\n') ? p_eol + 1 : p_eol;
    ...
    static int STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState *obj, int pos, const char *new_text, int new_text_len) {
        ...
        if (!is_resizable && new_text_len > avail)
            new_text_len = (int) (ImTextFindValidUtf8CodepointEnd(new_text, new_text + new_text_len, new_text + avail) -
                                  new_text);
        // Truncate to closest UTF-8 codepoint. Alternative: return 0 to cancel insertion.
        if (new_text_len == 0)
            return 0;
...
void ImGuiInputTextState::OnKeyPressed(int key) {
    ...
    if (key_u == STB_TEXTEDIT_K_LEFT || key_u == STB_TEXTEDIT_K_LINESTART || key_u == STB_TEXTEDIT_K_TEXTSTART || key_u
        == STB_TEXTEDIT_K_BACKSPACE || key_u == STB_TEXTEDIT_K_WORDLEFT)
...
static bool InputTextFilterCharacter(ImGuiContext *ctx, ImGuiInputTextState *state, unsigned int *p_char,
    ...
    if (apply_named_filters && (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsHexadecimal |
                                         ...
                                         ImGuiInputTextFlags_CharsScientific | (ImGuiInputTextFlags)
                                         ImGuiInputTextFlags_LocalizeDecimalPoint))) {
...
static ImVec2 InputTextLineIndexGetPosOffset(ImGuiContext &g, ImGuiInputTextState *state, ImGuiTextIndex *line_index,
    ...
    if (it > it_begin)
        if (it == it_end || *it != cursor_n || (state != NULL && state->WrapWidth > 0.0f && state->LastMoveDirectionLR
                                                == ImGuiDir_Right && cursor_ptr[-1] != '\n' && cursor_ptr[-1] != 0))
            it--;
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf, int buf_size, const ImVec2 &size_arg,
    ...
    if (is_multiline) {
        ...
        // Mimic some of ItemAdd() logic + add InputTextDeactivatedState.ID check.
    ...
    // state != NULL means its our state.
    const bool init_make_active = (input_requested_by_user || input_requested_by_nav || input_requested_by_reactivate ||
                                   user_scroll_finish);
    if (init_reload_from_user_buf) {
        ...
        // we use +1 to make sure that .Data is always pointing to at least an empty string.
        ...
        if (recycle_state && !init_changed_specs_readonly && (
                state->TextLen != buf_len || (state->TextA.Data == NULL || strncmp(state->TextA.Data, buf, buf_len) !=
                                              0)))
            recycle_state = false;
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        const bool is_cut = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_X, f_repeat, id) ||
                             Shortcut(ImGuiMod_Shift | ImGuiKey_Delete, f_repeat, id)) && !is_readonly && !is_password
                            && (!is_multiline || state->HasSelection());
        const bool is_copy = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_C, 0, id) ||
                              Shortcut(ImGuiMod_Ctrl | ImGuiKey_Insert, 0, id)) && !is_password && (
                                 !is_multiline || state->HasSelection());
        ...
        const bool is_redo = (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Y, f_repeat, id) || Shortcut(
                                  ImGuiMod_Ctrl | ImGuiMod_Shift | ImGuiKey_Z, f_repeat,
        ...
        const bool is_ctrl_enter = Shortcut(ImGuiMod_Ctrl | ImGuiKey_Enter, f_repeat, id) || Shortcut(
                                       ImGuiMod_Ctrl | ImGuiKey_KeypadEnter, f_repeat, id);
        const bool is_shift_enter = Shortcut(ImGuiMod_Shift | ImGuiKey_Enter, f_repeat, id) || Shortcut(
                                        ImGuiMod_Shift | ImGuiKey_KeypadEnter, f_repeat, id);
        ...
        const bool is_cancel = Shortcut(ImGuiKey_Escape, f_repeat, id) || (nav_gamepad_active && Shortcut(
                                                                               ImGuiKey_NavGamepadCancel, f_repeat,
                                                                               id));
    ...
    if (is_displaying_hint) {
        ...
        if (render_cursor || render_selection || g.ActiveId == id)
            ...
            // Inactive multi-line: end of buffer will be output by InputTextLineIndexBuild() special strchr() path.
        else
    ...
    if (render_cursor || render_selection) {
        ...
        if (render_selection) {
            ...
            // FIXME-DPI: those offsets should be part of the style? they don't play so well with multi-line selection.
    ...
    if (render_cursor) {
        ...
        if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
            draw_window->DrawList->AddLineV(cursor_screen_rect.Min.x, cursor_screen_rect.Min.y,
                                            ...
                                            1.0f * (float) (int) style._MainScale);
        ...
        if (!is_readonly && g.ActiveId == id) {
            ...
            // (this is a public struct, passed to io.Platform_SetImeDataFn() handler)
...
bool ImGui::ColorEdit4(const char *label, float col[4], ImGuiColorEditFlags flags) {
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (
        ...
        const bool draw_color_marker = (flags & (ImGuiColorEditFlags_DisplayHSV | ImGuiColorEditFlags_NoColorMarkers))
                                       == 0;

        const bool hide_prefix = draw_color_marker || (w_per_component <= CalcTextSize(
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg), rounding,
                                      1.0f * (float) (int) g.Style._MainScale);
...
static void TreeNodeStoreStackData(ImGuiTreeNodeFlags flags, float x1) {
    ...
    tree_node_data->DrawLinesTableColumn = (draw_lines && g.CurrentTable)
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label, const char *label_end) {
    ...
    const bool span_all_columns_label = (flags & ImGuiTreeNodeFlags_LabelSpanAllColumns) != 0 && (
                                            g.CurrentTable != NULL);
    ImRect frame_bb;
...
void ImGui::TreeNodeDrawLineToTreePop(const ImGuiTreeNodeStackData *data) {
    ...
    if (data->TreeFlags & ImGuiTreeNodeFlags_DrawLinesFull) {
        ...
        if (y2 + (g.Style.ItemSpacing.y + g.Style.TreeLinesRounding) < y2_full)
            // FIXME: threshold to use ToNodes Y2 instead of Full Y2 when close by ItemSpacing.y
            y2 = y2_full;
    }
...
bool ImGui::Selectable(const char *label, bool selected, ImGuiSelectableFlags flags, const ImVec2 &size_arg) {
    ...
    if (is_visible) {
        ...
        if (highlighted || selected) {
            ...
            ImU32 col = GetColorU32((held && highlighted)
                                        ? ImGuiCol_HeaderActive
                                        : highlighted
                                              ? ImGuiCol_HeaderHovered
                                              : ImGuiCol_Header);
...
ImGuiTypingSelectRequest *ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags) {
    ...
    if ((flags & ImGuiTypingSelectFlags_AllowBackspace) && IsKeyPressed(ImGuiKey_Backspace, ImGuiInputFlags_Repeat)) {
        char *p = (char *) (void *)
                ImTextFindPreviousUtf8Codepoint(data->SearchBuffer, data->SearchBuffer + buffer_len);
        *p = 0;
...
bool ImGui::BeginBoxSelect(const ImRect &scope_rect, ImGuiWindow *window, ImGuiID box_select_id,
    ...
    if (ms_flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d)) {
        ...
        if (ms_flags & ImGuiMultiSelectFlags_BoxSelect2d)
            if (bs->BoxSelectRectPrev.Min.x != bs->BoxSelectRectCurr.Min.x || bs->BoxSelectRectPrev.Max.x != bs->
                BoxSelectRectCurr.Max.x) {
                bs->UnclipRects[0].AddY(box_select_intersection.Min.y);
                bs->UnclipRects[0].AddY(box_select_intersection.Max.y);
            }
        if (ms_flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d))
            if (bs->BoxSelectRectPrev.Min.y != bs->BoxSelectRectCurr.Min.y || bs->BoxSelectRectPrev.Max.y != bs->
                BoxSelectRectCurr.Max.y) {
        ...
        if (!bs->UnclipRect.IsInverted() && (!window->ClipRect.Contains(bs->UnclipRect.Min) || !window->ClipRect.
                                             Contains(bs->UnclipRect.Max))) // !! Don't use Contains(ImRect)
            bs->UnclipMode = true;
...
void ImGui::EndBoxSelect(const ImRect &scope_rect, ImGuiMultiSelectFlags ms_flags) {
    ...
    // FIXME-MULTISELECT FIXME-DPI: Styling
...
static void DebugLogMultiSelectRequests(const char *function, const ImGuiMultiSelectIO *io) {
    ...
    for (const ImGuiSelectionRequest &req: io->Requests) {
        ...
        if (req.Type == ImGuiSelectionRequestType_SetRange)
            IMGUI_DEBUG_LOG_SELECTION(
                "[selection] %s: Request: SetRange %" IM_PRId64 "..%" IM_PRId64 " (0x%" IM_PRIX64 "..0x%" IM_PRIX64
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData *ms, ImGuiWindow *window) {
    if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect) {
        ...
        scope_rect.Min = ImMin(scope_rect.Min + ImVec2(window->DecoInnerSizeX1, window->DecoInnerSizeY1),
                               scope_rect.Max);
        return scope_rect;
    }
}
...
ImGuiMultiSelectIO *ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, int selection_size, int items_count) {
    ...
    // CalcScopeRect() for ImGuiMultiSelectFlags_ScopeRect will measure in EndMultiSelect().
    ...
    if (g.NavJustMovedToId != 0 && g.NavJustMovedToFocusScopeId == ms->FocusScopeId && g.
...
ImGuiMultiSelectIO *ImGui::EndMultiSelect() {
    ...
    IM_ASSERT(
        g.MultiSelectTempDataStacked > 0 && &g.MultiSelectTempData[g.MultiSelectTempDataStacked - 1] == g.
        CurrentMultiSelect);
    ...
    if (ms->IsFocused) {
        ...
        if (ms->IO.RangeSrcReset || (ms->RangeSrcPassedBy == false && ms->IO.RangeSrcItem !=
                                     ImGuiSelectionUserData_Invalid))
        // Can't read storage->RangeSrcItem here -> we want the state at beginning of the scope (see tests for easy failure)
        {
        ...
        if ((ms->Flags & (ImGuiMultiSelectFlags_BoxSelect1d | ImGuiMultiSelectFlags_BoxSelect2d)) && GetBoxSelectState(
                ms->BoxSelectId))
...
void ImGui::MultiSelectItemHeader(ImGuiID id, bool *p_selected, ImGuiButtonFlags *p_button_flags) {
    ...
    if (ms->IsFocused) {
        ...
        if (ms->IsKeyboardSetRange) {
            ...
            // Assume that g.NavJustMovedToId is not clipped.
            if (is_range_dst)
        ...
        if (ImGuiBoxSelectState *bs = GetBoxSelectState(ms->BoxSelectId)) {
            ...
            if (!window->DC.NavIsScrollPushableX) // FIXME: Rename to be more generic.
                if (ImGuiTable *table = g.CurrentTable)
                    if (table->CurrentColumn != -1) {
                        ...
                        // When zero sized we expect that bounds have been clamped and thus are unreliable
                        {
            ...
            if ((rect_overlap_curr && !rect_overlap_prev && !selected) || (rect_overlap_prev && !rect_overlap_curr)) {
                if (storage->LastSelectionSize <= 0 && bs->IsStartedSetNavIdOnce) {
                    pressed = true;
                    // First item act as a pressed: code below will emit selection request and set NavId (whatever we emit here will be overridden anyway)
...
void ImGui::MultiSelectAddSetRange(ImGuiMultiSelectTempData *ms, bool selected, int range_dir,
    ...
    if (ms->IO.Requests.Size > 0 && first_item == last_item && (ms->Flags & ImGuiMultiSelectFlags_NoRangeSelect) == 0) {
        ...
        if (prev->Type == ImGuiSelectionRequestType_SetRange && prev->RangeLastItem == ms->PrevSubmittedItem && prev->
            Selected == selected) {
    ...
    ImGuiSelectionRequest req = {
        ImGuiSelectionRequestType_SetRange, selected, (ImS8) range_dir, (range_dir > 0) ? first_item : last_item,
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) && (
        ...
        if (nav_earliest_child->ParentWindow == window && nav_earliest_child->DC.ParentLayoutType ==
            ...
            if (g.NavCursorVisible) {
                g.NavCursorVisible = false;
                // Hide nav cursor for the current frame so we don't see the intermediary selection. Will be set again
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled) {
    ...
    // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (menu_is_open && !hovered && g.HoveredWindow == window && !moving_toward_child_menu && !g.
            NavHighlightItemUnderNav && g.ActiveId == 0)
            want_close = true;
    ...
    if (menu_is_open) {
        ...
        menu_is_open = BeginPopupMenuEx(id, label, window_flags);
        // menu_is_open may be 'false' when the popup is completely clipped (e.g. zero size display)
        PopStyleVar();
        if (menu_is_open) {
...
void ImGui::EndMenu() {
    ...
    IM_ASSERT_USER_ERROR_RET(
        (window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu)) == (
            ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu), "Calling EndMenu() in wrong window!");
...
bool ImGui::MenuItemEx(const char *label, const char *icon, const char *shortcut, bool selected, bool enabled) {
    ...
    const ImGuiSelectableFlags selectable_flags =
            (ImGuiSelectableFlags) ImGuiSelectableFlags_SelectOnRelease | (ImGuiSelectableFlags)
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar) {
    ...
    const float shrink_min_width = (tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyMixed)
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++) {
        ...
        section->WidthAfterShrinkMinWidth += ImMin(tab->ContentWidth, shrink_min_width) + (section_n == curr_section_n
            ? tab_spacing
            : 0.0f);
    ...
    for (int section_n = 0; section_n < 3; section_n++) {
        ...
        width_all_tabs_after_min_width_shrink += sections[section_n].WidthAfterShrinkMinWidth + sections[section_n].
                Spacing;
    }
    ...
    tab_bar->ScrollButtonEnabled = ((width_all_tabs_to_use_for_scroll > tab_bar->BarRect.GetWidth() && tab_bar->Tabs.
                                     Size > 1) && !(tab_bar->Flags & ImGuiTabBarFlags_NoTabListScrollingButtons) &&
                                    can_scroll);
...
bool ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open, ImGuiTabItemFlags flags,
    ...
    ImGuiButtonFlags button_flags = ((ImGuiButtonFlags)(is_tab_button
                                                            ? ImGuiButtonFlags_PressedOnClickRelease
                                                            : ImGuiButtonFlags_PressedOnClick) |
    ...
    if (hovered && g.HoveredIdNotActiveTimer > TOOLTIP_DELAY && bb.GetWidth() < tab->ContentWidth) {
        ...
        bb.Max.x = bb.Min.x + IM_TRUNC(ImLerp(bb.GetWidth(), tab->ContentWidth,
    ...
    if (is_visible) {
        ...
        if (tab_contents_visible && (tab_bar->Flags & ImGuiTabBarFlags_DrawSelectedOverline) && style.TabBarOverlineSize
            > 0.0f) {
            ...
            ImU32 overline_col = GetColorU32(tab_bar_focused
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList *draw_list, const ImRect &bb, ImGuiTabItemFlags flags,
    ...
    bool is_hovered = g.HoveredId == tab_id || g.HoveredId == close_button_id || g.ActiveId == tab_id || g.ActiveId ==
    ...
    if (close_button_id != 0) {
        if (is_contents_visible)
            close_button_visible = (g.Style.TabCloseButtonMinWidthSelected < 0.0f)
                                       ? true
                                       : (is_hovered && bb.GetWidth() >= ImMax(
                                              button_sz, g.Style.TabCloseButtonMinWidthSelected));
        else
            close_button_visible = (g.Style.TabCloseButtonMinWidthUnselected < 0.0f)
                                       ? true
                                       : (is_hovered && bb.GetWidth() >= ImMax(
                                              button_sz, g.Style.TabCloseButtonMinWidthUnselected));
    }
    ...
    if (close_button_visible || unsaved_marker_visible) {
        const bool visible_without_hover = unsaved_marker_visible || (is_contents_visible
